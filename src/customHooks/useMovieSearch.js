...
const useMovieSearch = (props) => {
    ...
    const handleSearchClick = async () => {
        ...
        try {
            ...
            `${process.env.REACT_APP_BASE_URL}api/movies/gemini/recommend?query=${encodeURIComponent(
                text.current.value
            )}`
            );
            const data = await response.json();
            console.log("Recommended Movies:", data);
            dispatch(addRecommendedMovies({ movieNames: data.movieNames, movies: data.movies }));
            successMessage("Movies fetched!");
        } catch (err) {
